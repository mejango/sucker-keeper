...
- **Funding**: API registration + pay-to-address. Deposits are ETH sent to the keeper's wallet on any supported chain, claimed by tx hash.
- **Threshold**: percentage, per registration (default 1%).
...
- Graph: nodes = project's chains; directed edge X→Y exists where X has a sucker whose peer is on Y (parallel native/CCIP edges are common — Dijkstra picks the cheaper). Edge weight = probed transport fee + a flat hop penalty so free OP edges still prefer fewer hops.
